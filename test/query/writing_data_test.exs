defmodule WritingDataTest do
  ...
  test "insert" do
    ...
    {:ok, %Collection{data: [%{"id" => ^key, "name" => "Hello", "attr" => "World"}]}} =
      run(table_query)
  end
  ...
  test "insert conflict options" do
    ...
    q =
      insert(table_query, [%{id: id, value: 3}], %{
        conflict: fn _id, old, new ->
          merge(old, %{value: add(get_field(old, "value"), get_field(new, "value"))})
        end
      })
